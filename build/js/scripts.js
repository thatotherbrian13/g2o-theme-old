var G2OBundle = (() => {
  ...
  var PropTween = /* @__PURE__ */ (function() {
    function PropTween3(next, target, prop, start2, change, renderer, data2, setter, priority) {
      this.t = target;
      this.s = start2;
      this.c = change;
      this.p = prop;
  ...
  function getRotationAtBezierT(segment, i, t) {
    ...
    b += (c - b) * t;
    a += (b - a) * t;
    return _round5(_atan22(b + (c + (segment[i + 7] - c) * t - b) * t - a, x) * _RAD2DEG2);
  }
  function sliceRawPath(rawPath, start2, end) {
    end = _isUndefined3(end) ? 1 : _roundPrecise3(end) || 0;
    start2 = _roundPrecise3(start2) || 0;
    var loops = Math.max(0, ~~(_abs2(end - start2) - 1e-8)), path = copyRawPath(rawPath);
    if (start2 > end) {
      start2 = 1 - start2;
      end = 1 - end;
    ...
    var wrap3 = end > 1, s = getProgressData(path, start2, _temp, true), e = getProgressData(path, end, _temp2), eSeg = e.segment, sSeg = s.segment, eSegIndex = e.segIndex, sSegIndex = s.segIndex, ei = e.i, si = s.i, sameSegment = sSegIndex === eSegIndex, sameBezier = ei === si && sameSegment, wrapsBehind, sShift, eShift, i, copy, totalSegments, l, j;
    if (wrap3 || loops) {
  ...
  function measureSegment(segment, startIndex, bezierQty) {
    ...
    for (j = startIndex + 2; j < endIndex; j += 6) {
      ...
      y2 = segment[j + 1] - y1;
      xd = xd1 = yd = yd1 = 0;
      if (_abs2(x4) < 0.01 && _abs2(y4) < 0.01 && _abs2(x2) + _abs2(y2) < 0.01) {
        if (segment.length > 8) {
          segment.splice(j, 6);
          j -= 6;
          endIndex -= 6;
  ...
  function getPositionOnPath(rawPath, progress, includeAngle, point) {
    ...
    i = ~~(i / resolution) * 6;
    a = segment[i];
    result.x = _round5((t * t * (segment[i + 6] - a) + 3 * inv * (t * (segment[i + 4] - a) + inv * (segment[i + 2] - a))) * t + a);
    result.y = _round5((t * t * (segment[i + 7] - (a = segment[i + 1])) + 3 * inv * (t * (segment[i + 5] - a) + inv * (segment[i + 3] - a))) * t + a);
    if (includeAngle) {
  ...
  function pointsToSegment(points, curviness) {
    _abs2(points[0] - points[2]) < 1e-4 && _abs2(points[1] - points[3]) < 1e-4 && (points = points.slice(2));
  ...
  var _segmentToRawPath = function _segmentToRawPath2(plugin2, segment, target, x, y, slicer, vars, unitX, unitY) {
    if (vars.type === "cubic") {
      segment = [segment];
    } else {
  ...
  var _getAlignMatrix = function _getAlignMatrix2(fromElement, toElement, fromOrigin, toOrigin) {
    ...
    if (toOrigin === "auto" && toElement.getTotalLength && toElement.tagName.toLowerCase() === "path") {
      p = toElement.getAttribute("d").match(_numExp2) || [];
      p = m.apply({
        x: +p[0],
        y: +p[1]
      });
  ...
  var _addDimensionalPropTween = function _addDimensionalPropTween2(plugin2, target, property, rawPath, pathProperty, forceUnit) {
    var cache = target._gsap, harness = cache.harness, alias = harness && harness.aliases && harness.aliases[property], prop = alias && alias.indexOf(",") < 0 ? alias : property, pt = plugin2._pt = new PropTween2(plugin2._pt, target, prop, 0, 0, _emptyFunc3, 0, cache.set(target, prop, plugin2));
    pt.u = _getUnit(cache.get(target, prop, forceUnit)) || 0;
    pt.path = rawPath;
    pt.pp = pathProperty;
    plugin2._props.push(prop);
  };
  var _sliceModifier = function _sliceModifier2(start2, end) {
  ...
  var MotionPathPlugin = {
    ...
    render: function render3(ratio, data2) {
      ...
      if (data2.tween._time || !_reverting3()) {
        if (ratio > 1) {
          ratio = 1;
        } else if (ratio < 0) {
          ratio = 0;
        ...
        data2.rotate && data2.rSet(data2.target, data2.rProp, rawPaths[0].angle * (data2.radians ? _DEG2RAD3 : 1) + data2.rOffset + data2.ru, data2, ratio);
      } else {
        data2.styles.revert();
      }
    },
  ...
  _getGSAP5() && gsap4.registerPlugin(MotionPathPlugin);
...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  window.ScrollTrigger = ScrollTrigger2;
  window.MotionPathPlugin = MotionPathPlugin;
  window.Swiper = Swiper;
  gsapWithCSS.defaults({ ease: "none" });
  gsapWithCSS.registerPlugin(MotionPathPlugin, ScrollTrigger2);
  ScrollTrigger2.defaults({
    markers: false
  });
  var containers = gsapWithCSS.utils.toArray(".reveal");
  ...
  mm.add("(min-width: 1024px)", () => {
    ...
    gsapWithCSS.set("#open", { autoAlpha: 0 });
    gsapWithCSS.to("#open", {
      autoAlpha: 1,
      duration: 0.5,
      ease: "none",
  ...
  var menu = gsapWithCSS.timeline({ paused: true, reversed: true });
  menu.to("#menu", {
    duration: 0.75,
    ease: "power2.inOut",
    left: 0
  ...
  function scrollToElement($el, time = 500) {
    const scrollTop = $el.getBoundingClientRect().top + window.pageYOffset - 200;
    const scrollOptions = {
      top: scrollTop,
      behavior: "smooth"
    };
  ...
  function setupAccordion() {
    ...
    const accordionSelector = accordionClasses.join(", ");
    function toggleAccordion() {
      const $parent = this.parentNode;
      const isActive = $parent.classList.contains("active");
      const accordion = $parent.closest(accordionSelector);
      if (!accordion) return;
      accordion.querySelectorAll(".accordion-item.active").forEach(
  ...
  function setupAccordionSpread() {
    ...
    function toggleAccordionSpread(event2) {
